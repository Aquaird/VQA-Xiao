...
class TestWrapper(object):
    ...
    def test_model(self, sess):
        ...
        test_word_count = [0.0 for p in range(5)]
        test_word_accuracy = [0.0 for p in range(5)]
        test_wup_count = [0.0 for p in range(5)]
        ...
        for i in range(cfg.TEST.NUMBER):
            ...
            test_word_count[result[0]] += 1
            if result[2] != -1:
                test_wup_count[result[0]] += 1
                test_wup_value[result[0]] += result[2]

        test_word_accuracy = list(map(lambda x,y: x/y, test_word_accuracy, test_word_count))
