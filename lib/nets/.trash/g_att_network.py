...
class Network(object):
    ...
    def _build_network(self, is_training=True):
        ...
        with tf.variable_scope('final'):
            # [batch_size, lstm_size]
            att_lstm = tf.reshape(tf.reduce_mean(self._feature_lstm, axis=1), [-1, 2*cfg.LSTM_SIZE])
            # [batch_size, t_att_size+lstm_size]
            w_class = tf.get_variable('w_class', [2*cfg.LSTM_SIZE, 1001], initializer=self.initializer)
            softmax_input = tf.matmul(att_lstm, w_class)
    ...
    def train_step(self, sess, handle):
        feed_dict = {self.handle:handle}
        qtype, word_pred_code, answer_code, word_pred_accuracy ,qa,oa, loss, _ = sess.run([
            self.qtype,
            self._predictions['word_pred'],
            self._answer_code,
            self._predictions['word_accuracy'],
