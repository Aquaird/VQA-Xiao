...
class Network(object):
    ...
    def _build_network(self, is_training=True):
        ...
        with tf.variable_scope('fusion'):
            wh = tf.get_variable('wh', (cfg.LSTM_SIZE, cfg.LSTM_SIZE), initializer=self.initializer, regularizer=self.weights_regularizer)
            wv = tf.get_variable('wv', (cfg.VIDEO_SIZE+cfg.QUES_SIZE, cfg.LSTM_SIZE), initializer=self.initializer, regularizer=self.weights_regularizer)
            b = tf.get_variable('b', (cfg.LSTM_SIZE), initializer=self.initializer, regularizer=self.weights_regularizer)

            fusion = tf.add(tf.matmul(tf.reshape(lstm_1_out, (-1, cfg.LSTM_SIZE)), wh),
                            tf.matmul(tf.reshape(qv_co_atten, (-1, cfg.VIDEO_SIZE+cfg.QUES_SIZE)), wv))
            fusion = tf.reshape(fusion + b, (-1, cfg.FRAME_NUMBER, cfg.LSTM_SIZE))

        lstm_2_out = self._build_lstm(fusion, cfg.LSTM_SIZE, 'lstm2')

        softmax_input = lstm_2_out[:,-1]
        with tf.variable_scope('final'):

            w_2 = tf.get_variable('w_class_2', [cfg.LSTM_SIZE, cfg.VOCAB_SIZE], initializer=self.initializer, regularizer=self.weights_regularizer)
            softmax_input_2 = tf.matmul(softmax_input, w_2)

            w_1 = tf.get_variable('w_class_1', [cfg.LSTM_SIZE, cfg.VOCAB_SIZE], initializer=self.initializer, regularizer=self.weights_regularizer)
            softmax_input_1 = tf.matmul(tf.reshape(lstm_1_out[:,-1], (-1, cfg.LSTM_SIZE)), w_1)

            softmax_output = tf.nn.softmax((softmax_input_1+softmax_input_2)/2, dim=-1)
